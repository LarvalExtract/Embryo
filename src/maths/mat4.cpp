...
// New matrix stuff
template <class T>
Matrix4x4<float> Matrix4x4<T>::Identity()
{
...
template <class T>
Matrix4x4<float> Matrix4x4<T>::Translation(const Vec3<float> &translate)
{
	Matrix4x4<float> result(1.0f);
...
template <class T>
Matrix4x4<float> Matrix4x4<T>::Rotation(float angle, const Vec3<float> &axis)
{
	Matrix4x4<float> result;
	...
	result.elements[0][0] = axis.x * omc + cosR;
	result.elements[0][1] = axis.y * axis.x * omc + axis.z * sinR;
	result.elements[0][2] = axis.x * axis.z * omc - axis.y * sinR;

	result.elements[1][0] = axis.x * axis.y * omc - axis.z * sinR;
	result.elements[1][1] = axis.y * omc + cosR;
	result.elements[1][2] = axis.y * axis.z * omc + axis.x * sinR;

	result.elements[2][0] = axis.z * axis.x * omc + axis.y * sinR;
	result.elements[2][1] = axis.y * axis.z * omc - axis.x * sinR;
	result.elements[2][2] = axis.z * omc + cosR;
...
template <class T>
Matrix4x4<float> Matrix4x4<T>::Scale(const Vec3<float> &scale)
{
	Matrix4x4<float> result;
...
template <class T>
Matrix4x4<float> Matrix4x4<T>::LookAt(Vec3<float> pos, const Vec3<float> &target, const Vec3<float> &up)
{
	...
	result.elements[3][0] = -Maths::Dot(xaxis, pos);
	result.elements[3][1] = -Maths::Dot(yaxis, pos);
	result.elements[3][2] = -Maths::Dot(zaxis, pos);
...
template <class T>
Matrix4x4<float> Matrix4x4<T>::Orthographic(float left, float right, float bottom, float top, float near, float far)
{
	Matrix4x4<float> result;
...
template <class T>
Matrix4x4<float> Matrix4x4<T>::Perspective(float fov, float aspectRatio, float near, float far)
{
	Matrix4x4<float> result;
...
template <class T>
Matrix4x4<T>& Matrix4x4<T>::Add(Matrix4x4<T> lhs, const Matrix4x4<T> &rhs)
{
...
template <class T>
Matrix4x4<T>& Matrix4x4<T>::Subtract(Matrix4x4<T> lhs, const Matrix4x4<T> &rhs)
{
	for (char c = 0; c < COLUMN_SIZE; c++)
		for (char r = 0; r < ROW_SIZE; r++)
			lhs.elements[c][r] -= rhs.elements[c][r];
...
template <class T>
Matrix4x4<T>& Matrix4x4<T>::Multiply(Matrix4x4<T> lhs, const Matrix4x4<T> &rhs)
{
	for (char m = 0; m < MATRIX_SIZE; m++)
		for (char c = 0; c < COLUMN_SIZE; c++)
			for (char r = 0; r < ROW_SIZE; r++)
